...
class Mensagem extends CI_Controller {
    ...
    public function index() {
        ...
        $data['URLADICIONAR'] = site_url('cliente/mensagem/nova');

        $this->parser->parse('cliente/mensagem', $data);
    }
    ...
    public function nova() {
        ...
        $this->parser->parse('cliente/mensagem_form', $data);
    }
    ...
    public function salvar() {
        ...
        if (!$erros) {
            ...
            $email_from = $this->db->get_where('cliente', array(
                        'idcliente' => $this->session->userdata('idusuario')
                    ))->row()->email;

            $nome = $this->db->get_where('cliente', array(
                        'idcliente' => $this->session->userdata('idusuario')
                    ))->row()->nome;

            $this->EmailM->notificacao_mensagem_enviada_email_usuario('cliente', $email_from, $nome);

            redirect('cliente/mensagem/nova');
    ...
    public function lida($codigo_mensagem = '') {
        ...
        $this->parser->parse('cliente/mensagem_form_responder', $data);
    }
    ...
    public function responder($codigo_mensagem) {
        ...
        if (!$erros) {
            ...
            redirect('cliente/mensagem/lida');
        } else {
            $this->session->set_flashdata('erro', nl2br($mensagem));
