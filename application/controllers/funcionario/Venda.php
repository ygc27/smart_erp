...
class Venda extends CI_Controller {
    ...
    public function index() {
        ...
        $data['URLADICIONAR'] = site_url('funcionario/venda/adicionar');
        $data['URLLISTAR'] = site_url('funcionario/venda');

        $data['vendas'] = $this->VendaM->get_all()->result();

        $this->parser->parse('funcionario/venda', $data);
    }

    public function adicionar() {
        $data = array();
        $data['pagina_nome'] = 'historico-venda';
        $data['ACAO'] = 'Novo Venda';
        ...
        $this->parser->parse('funcionario/venda_form', $data);
    }
    ...
    public function get_desconto() {
        ...
        if (!empty($id_cliente)) {
            $cliente_desconto = $this->db->get_where('cliente', array('idcliente' => $id_cliente))->row()->desconto;
    ...
    public function get_produto_selecionado($tipo = '', $idproduto = '', $total_linha = '') {
        if ($tipo == 'mouse') {
            $produto = $this->db->get_where('produto', array('idproduto' => $idproduto))->row();
    ...
    public function get_subcategoria($idcategoria = '') {
        if (!empty($idcategoria)) {
            ...
            $subcategorias = $this->db->get_where('subcategoria', array('idcategoria' => $idcategoria))->result();
    ...
    public function salvar() {
        ...
        if (!$erros) {
            ...
            redirect('funcionario/venda');
        } else {
            $this->session->set_flashdata('erro', nl2br($mensagem));

            redirect('funcionario/venda/adicionar');
        }
    }
    ...
    public function visualizar($idvenda) {
        ...
        $this->parser->parse('funcionario/venda_visualizar', $data);
    }
